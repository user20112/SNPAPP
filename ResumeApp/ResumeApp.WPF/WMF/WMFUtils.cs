...
namespace ResumeApp.WPF.Classes.WMF
{
    public class WMFUtils
    {
        ...
        public static UInt64 AddSecondsTo100nsTime(int secondsToAdd, UInt64 presentationClockin100ns, UInt64 maxPresentationTime100ns)
        {
            ...
            // do the math
            tmpClockin100ns += secondsToAddIn100ns;
            // check for negative
        ...
        public static HResult CheckMediaType(IMFMediaType pmt, Guid gMajorType, Guid gSubtype)
        {
            ...
            pmt.GetGUID(MFAttributesClsid.MF_MT_MAJOR_TYPE, out Guid major_type);
            if (major_type == gMajorType)
            {
                // Get the subtype GUID.
                pmt.GetGUID(MFAttributesClsid.MF_MT_SUBTYPE, out Guid subtype);
                if (subtype != gSubtype)
                {
        ...
        public static HResult ConfigureSourceReaderWithVideoFormat(IMFSourceReaderAsync sourceReader, MFVideoFormatContainer videoFormatContainer)
        {
            HResult hr;
            ...
            try
            {
                ...
                // temporary MFVideoFormatContainer. Once we have that we compare it against the input
                ...
                for (int typeIndex = 0; typeIndex < maxFormatsToTestFor; typeIndex++)
                {
                    ...
                    hr = sourceReader.GetNativeMediaType(MF_SOURCE_READER_FIRST_VIDEO_STREAM, typeIndex, out IMFMediaType mediaTypeObj);
                    ...
                    MFVideoFormatContainer tmpContainer = MediaTypeInfo.GetVideoFormatContainerFromMediaTypeObject(mediaTypeObj, videoFormatContainer.VideoDevice);
                    ...
                    if (videoFormatContainer.CompareTo(tmpContainer) == 0)
                    {
                        ...
                        hr = sourceReader.SetCurrentMediaType(MF_SOURCE_READER_FIRST_VIDEO_STREAM, null, mediaTypeObj);
                        if (hr != HResult.S_OK)
                        {
                            // we failed
        ...
        /// <returns>S_OK for success, nz for fail</returns>
        public static HResult ConfigureSourceReaderWithVideoFormat(IMFSourceReaderAsync sourceReader, List<Guid> subTypes)
        {
            HResult hr;
            ...
            try
            {
                ...
                hr = sourceReader.GetNativeMediaType(MF_SOURCE_READER_FIRST_VIDEO_STREAM, 0, out IMFMediaType mediaTypeObj);
                if (hr != HResult.S_OK)
                {
                    // we failed
                    throw new Exception("ConfigureSourceReaderWithVideoFormat failed on call to GetNativeMediaType, retVal=" + hr.ToString());
        ...
        public static string ConvertGuidToName(Guid guidToConvert)
        {
            ...
            if (guidToConvert == MFAttributesClsid.MF_CAPTURE_METADATA_FACEROIS) return "MF_CAPTURE_METADATA_FACER";
        ...
        public static string ConvertPropVariantToString(PropVariant propVariant, bool appendGuidToGuidNames)
        {
            ...
            if (propVariant.GetVariantType() == ConstPropVariant.VariantType.Guid)
            {
                ...
                {
                    return ConvertGuidToName(pvGuid) + " (" + pvGuid.ToString() + ")";
                }
                else
                {
                    return ConvertGuidToName(pvGuid);
                }
            }
        ...
        /// </summary>
        /// <param name="rangeValue">a value between 0 and WMFUtils.MAX_DURATION_RANGE representing the
        ...
        {
            return (videoDuration * (UInt64)rangeValue) / MAX_DURATION_RANGE;
        }
        ...
        /// <summary>
        /// Converts a video position to a range value between 0 and WMFUtils.MAX_DURATION_RANGE
        /// </summary>
        ...
        public static int ConvertVideoPostionToRangeValue(UInt64 videoDuration, UInt64 videoPosition)
        {
            ...
            return (int)(((UInt64)MAX_DURATION_RANGE * videoPosition) / videoDuration);
        }
        ...
        public static IMFSample CreateMediaSampleFromBuffer(long sourceSampleTimeStamp, long sourceSampleDuration, IMFMediaBuffer sourceSampleBuffer, int sourceSampleSize, IMFAttributes sourceAttributes)
        {
            IntPtr srcRawDataPtr;
            bool srcIs2D = false;
            ...
            try
            {
                ...
                if ((sourceSampleBuffer is IMF2DBuffer) == false)
                {
                    ...
                    LockIMFMediaBufferAndGetRawData(sourceSampleBuffer, out srcRawDataPtr, out int maxLen, out int currentLen);
                    // now make the call to the version of this function which accepts only IntPtrs as the source
                    return CreateMediaSampleFromIntPtr(sourceSampleTimeStamp, sourceSampleDuration, srcRawDataPtr, sourceSampleSize, sourceAttributes);
                }
                else
                {
                    // we are an IMF2DBuffer, we get the stride here as well
                    LockIMF2DBufferAndGetRawData((sourceSampleBuffer as IMF2DBuffer), out srcRawDataPtr, out int srcStride);
                    srcIs2D = true;
                    // now make the call to the version of this function which accepts only IntPtrs as the source
                    return CreateMediaSampleFromIntPtr(sourceSampleTimeStamp, sourceSampleDuration, srcRawDataPtr, sourceSampleSize, sourceAttributes);
                }
            ...
            {
                if (srcIs2D == false) UnLockIMFMediaBuffer(sourceSampleBuffer);
                else UnLockIMF2DBuffer((sourceSampleBuffer as IMF2DBuffer));
            }
        }
        ...
        /// <param name="sourceAttributes">the attributes for the sample - we copy these</param>
        public static IMFSample CreateMediaSampleFromIntPtr(long sourceSampleTimeStamp, long sourceSampleDuration, IntPtr sourceSampleIntPtr, int sourceSampleSize, IMFAttributes sourceAttributes)
        {
            HResult hr;
            ...
            try
            {
                ...
                LockIMFMediaBufferAndGetRawData(outputBuffer, out IntPtr destRawDataPtr, out int maxLen, out int currentLen);
            ...
            {
                UnLockIMFMediaBuffer(outputBuffer);
                if (outputBuffer != null)
                {
        ...
        {
            MFExtern.MFCreateMediaType(out IMFMediaType ppmt);
            ppmt.SetGUID(MFAttributesClsid.MF_MT_MAJOR_TYPE, gMajorType);
            ppmt.SetGUID(MFAttributesClsid.MF_MT_SUBTYPE, gSubtype);
            return ppmt;
        }
        ...
        public static IMFTopologyNode CreateSinkNodeForStream(IMFMediaSink pSink, int streamIndex)
        {
            ...
            try
            {
                ...
                // get the StreamSink
                hr = pSink.GetStreamSinkByIndex(streamIndex, out IMFStreamSink pStream);
                if (hr != HResult.S_OK)
                {
        ...
        public static IMFSourceReaderAsync CreateSourceReaderAsyncFromDevice(MFDevice sourceDevice, IMFSourceReaderCallback asyncCallBackHandlerIn)
        {
            HResult hr;
            ...
            try
            {
                ...
                mediaSource = GetMediaSourceFromDevice(sourceDevice);
                if (mediaSource == null)
                {
                ...
                hr = MFExtern.MFCreateSourceReaderFromMediaSource(mediaSource, attributeContainer, out IMFSourceReader sourceReader);
                if (hr != HResult.S_OK)
                {
                    // we failed
        ...
        public static HResult EnumerateAllAttributeNamesAndValuesAsText(IMFAttributes attributesContainer, List<string> attributesToIgnore, int maxAttributes, out List<string> attrNamesAndValuesList)
        {
            ...
            for (int attrIndex = 0; attrIndex < maxAttributes; attrIndex++)
            {
                ...
                HResult hr = attributesContainer.GetItemByIndex(attrIndex, out Guid guid, pv);
                ...
                string outName = ConvertGuidToName(guid);
        ...
        public static IMFMediaType GetCurrentMediaTypeFromStreamDescriptor(IMFStreamDescriptor streamDescriptor)
        {
            ...
            try
            {
                ...
                hr = pHandler.GetCurrentMediaType(out IMFMediaType outMediaType);
                if (hr != HResult.S_OK)
                {
        ...
        public static List<MFDevice> GetDevicesByCategory(Guid attributeType, Guid filterCategory)
        {
            // our return value
            List<MFDevice> outList = new List<MFDevice>();
            int numDevices = 0;
            HResult hr;
            IMFAttributes attributeContainer = null;
            try
            {
                ...
                hr = MFExtern.MFEnumDeviceSources(attributeContainer, out IMFActivate[] deviceArr, out numDevices);
                if (hr != HResult.S_OK)
                {
                    // we failed
                ...
                // are just strings. Later we can use WMFUtils.GetMediaSourceFromDevice to build a video source
                // from the symbolic name and the activator here will have long since been cleaned up and released.
                // add the devices to our list as MFDevices
                for (int i = 0; i < numDevices; i++)
                {
                    ...
                    // create the new MFDevice
                    outList.Add(new MFDevice(friendlyName, symbolicLinkName, filterCategory));
        ...
        public static UInt64 GetDurationFromPresentationDescriptor(IMFPresentationDescriptor sourcePresentationDescriptor)
        {
            ...
            sourcePresentationDescriptor.GetUINT64(MFAttributesClsid.MF_PD_DURATION, out long presentationDuration);
            ...
            try
            {
                ...
                for (int typeCounter = 0; typeCounter < MAX_TYPES_TESTED_PER_TRANSFORM; typeCounter++)
                {
                    try
                    {
                        ...
                        hr = transformObj.GetInputAvailableType(0, typeCounter, out IMFMediaType mediaType);
        ...
        public static HResult GetInputTypesForTransformFromActivator(IMFActivate activatorObject, out List<MFTRegisterTypeInfo> rtInfoList)
        {
            ...
            HResult hr = activatorObject.GetAllocatedBlob(MFAttributesClsid.MFT_INPUT_TYPES_Attributes, out IntPtr outBlob, out int outSize);
            if (hr != HResult.S_OK)
            {
                return HResult.E_FAIL;
            }
        ...
        public static Guid GetMajorMediaTypeFromPresentationDescriptor(IMFPresentationDescriptor presentationDescriptor, int streamIndex)
        {
            ...
            try
            {
                ...
                hr = presentationDescriptor.GetStreamDescriptorByIndex(streamIndex, out bool streamIsSelected, out streamDescriptor);
                if (hr != HResult.S_OK)
                {
        ...
        public static Guid GetMajorMediaTypeFromStreamDescriptor(IMFStreamDescriptor streamDescriptor)
        {
            ...
            try
            {
                ...
                hr = pHandler.GetMajorType(out Guid guidMajorType);
                if (hr != HResult.S_OK)
                {
        ...
        /// <summary>
        /// Returns a media source from the contents of a MFDevice
        /// </summary>
        /// <param name="sourceDevice">the source device</param>
        /// <returns>a IMFMediaSource or null for fail</returns>
        public static IMFMediaSource GetMediaSourceFromDevice(MFDevice sourceDevice)
        {
            IMFMediaSource mediaSource = null;
            HResult hr = 0;
            IMFAttributes attributeContainer = null;
            try
            {
                ...
                {
                    // we failed
                    throw new Exception("GetMediaSourceFromDevice sourceDevice == null");
                }
                if ((sourceDevice.SymbolicName == null) || (sourceDevice.SymbolicName.Length == 0))
                {
                    // we failed
                    throw new Exception("GetMediaSourceFromDevice failed null or bad symbolicLinkStr");
                }
                if (sourceDevice.DeviceType == Guid.Empty)
                {
                    // we failed
                    throw new Exception("GetMediaSourceFromDevice DeviceType == Guid.Empty");
                }
                // Initialize an attribute store. We will use this to
                ...
                {
                    // we failed
                    throw new Exception("GetMediaSourceFromDevice failed on call to MFCreateAttributes, retVal=" + hr.ToString());
                }
                if (attributeContainer == null)
                {
                    // we failed
                    throw new Exception("GetMediaSourceFromDevice failed on call to MFEnumDeviceSources, attributeContainer == null");
                }
                ...
                {
                    // we failed
                    throw new Exception("GetMediaSourceFromDevice failed setting up the attributes, retVal=" + hr.ToString());
                }
                ...
                {
                    // we failed
                    throw new Exception("GetMediaSourceFromDevice failed setting up the symbolic name, retVal=" + hr.ToString());
                }
                ...
                {
                    // we failed
                    throw new Exception("GetMediaSourceFromDevice failed on call to MFCreateDeviceSource, retVal=" + hr.ToString());
                }
            }
            finally
            {
        ...
        public static IMFMediaSource GetMediaSourceFromFile(string mediaFileName)
        {
            ...
            try
            {
                ...
                hr = pSourceResolver.CreateObjectFromURL(
                        ...
                        out object pSource                 // Receives a pointer to the media source.
                    );
                if (hr != HResult.S_OK)
                {
        ...
        public static IMFMediaType GetMediaTypeFromStreamDescriptorById(IMFStreamDescriptor streamDescriptor, int mediaTypeId)
        {
            ...
            try
            {
                ...
                hr = typeHandler.GetMediaTypeByIndex(mediaTypeId, out IMFMediaType outMediaType);
                if (hr != HResult.S_OK)
                {
        ...
        public static object GetObjectFromTopologyNode(IMFTopologyNode topoNode)
        {
            ...
            HResult hr = topoNode.GetObject(out object nodeObject);
        ...
        public static List<IMFMediaType> GetOutputMediaTypesFromTransformByGuid(Guid transformGuid, bool wantLocalServer)
        {
            ...
            try
            {
                ...
                for (int typeCounter = 0; typeCounter < MAX_TYPES_TESTED_PER_TRANSFORM; typeCounter++)
                {
                    ...
                    {
                        // get the available input type for the current typeCounter
                        hr = transformObj.GetOutputAvailableType(0, typeCounter, out IMFMediaType mediaType);
                        // not found, we are done
                        if (hr != HResult.S_OK) break;
                        if (mediaType == null) break;
                        // add it now
        ...
        public static HResult GetOutputTypesForTransformFromActivator(IMFActivate activatorObject, out List<MFTRegisterTypeInfo> rtInfoList)
        {
            ...
            // get the data from the activator. This comes out in a blob which is actually
            // an array of MFT_REGISTER_TYPE_INFO structs.
            HResult hr = activatorObject.GetAllocatedBlob(MFAttributesClsid.MFT_OUTPUT_TYPES_Attributes, out IntPtr outBlob, out int outSize);
            if (hr != HResult.S_OK)
            {
                return HResult.E_FAIL;
            }
        ...
        public static IMFStreamSink GetStreamSinkByMajorMediaType(IMFMediaSink mediaSink, Guid majorMediaType)
        {
            ...
            for (int streamIndex = 0; streamIndex < MAXSTREAMS; streamIndex++)
            {
                try
                {
                    ...
                    // get the major type
                    hr = workingMediaType.GetMajorType(out Guid workingMajorType);
                    if (hr != HResult.S_OK) continue;
        ...
        public static string GetStringForKeyFromActivator(IMFActivate activatorContainer, Guid guidKey)
        {
            ...
            // get it now.
            HResult hr = activatorContainer.GetAllocatedString(
                guidKey,
                out string allocatedStr,
                out _
                );
        ...
        public static bool IncrementAudioVolumeOnSession(IMFMediaSession mediaSession, float attenuationIncrement)
        {
            ...
            try
            {
                ...
                hr = simpleAudioService.GetMasterVolume(out float attenuationLevel);
                if (hr != HResult.S_OK)
                {
        ...
        public static bool IncrementPlaybackRateOnSession(IMFMediaSession mediaSession, float rateIncrement, out float newRate)
        {
            ...
            try
            {
                ...
                rateControlService = (rcServiceObj as IMFRateControl);
                // now get the current rate from the rate control interface
                hr = rateControlService.GetRate(ref isThinned, out float currentRate);
                if (hr != HResult.S_OK)
                {
                ...
                bool retBool = IsPlaybackRateSupported(mediaSession, newRate, false);
                if (retBool == false) return false;
                // ok we can do it
                // now set the current rate on the rate control interface
                hr = rateControlService.SetRate(false, newRate);
                if (hr != HResult.S_OK)
                {
        ...
        public static bool IsRewindSupported(IMFMediaSession mediaSession, bool wantThinned)
        {
            ...
            try
            {
                ...
                hr = rateSupportService.GetSlowestRate(MFRateDirection.Reverse, wantThinned, out float supportedRate);
                if (hr == HResult.MF_E_REVERSE_UNSUPPORTED)
                {
                    // just let the user know the rate is not supported
                    return false;
        ...
        public static HResult SetCurrentMediaTypeOnIMFStreamDescriptorByFormatContainer(IMFStreamDescriptor videoStreamDescriptor, MFVideoFormatContainer videoFormatContainer)
        {
            HResult hr;
            IMFMediaType workingMediaType = null;
            ...
            hr = videoStreamDescriptor.GetMediaTypeHandler(out IMFMediaTypeHandler workingMediaTypeHandler);
            if (hr != HResult.S_OK)
            {
            ...
            try
            {
                ...
                for (int subTypeIndex = 0; subTypeIndex < MAXSUBTYPES; subTypeIndex++)
                {
                    ...
                    hr = MediaTypeInfo.GetSupportedFormatsFromMediaType(workingMediaType, out Guid majorType, out Guid subType, out int attributeCount, out int frameSizeWidth, out int frameSizeHeight, out int frameRate, out int frameRateDenominator, out int frameRateMin, out int frameRateMinDenominator, out int frameRateMax, out int frameRateMaxDenominator);
                    if (hr != HResult.S_OK)
                    {
                    ...
                    if ((videoFormatContainer.MajorType != Guid.Empty) ||
                        (videoFormatContainer.SubType != subType) ||
                        (videoFormatContainer.FrameSizeWidth != 0) ||
                        (videoFormatContainer.FrameSizeHeight != 0))
                    {
                        // we are not interested in this media type, release it and get the next
                        if (workingMediaType != null)
                        {
        ...
        public static bool ToggleAudioMuteStateOnSession(IMFMediaSession mediaSession)
        {
            ...
            try
            {
                ...
                hr = simpleAudioService.GetMute(out bool muteState);
                if (hr != HResult.S_OK)
                {
        ...
        private static extern UInt32 CoCreateInstance([In, MarshalAs(UnmanagedType.LPStruct)] Guid rclsid,

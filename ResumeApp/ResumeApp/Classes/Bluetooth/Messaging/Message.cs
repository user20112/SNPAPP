...
namespace Robotics.Messaging
{
    ...
    public class Message
    {
        ...
        private const int BufferSize = 260;
        ...
        private static void AddAll(ByteList list, byte[] bytes)
        {
        ...
        private static byte[] Serialize(object[] arguments)
        {
            ...
            foreach (var a in arguments)
            {
                ...
                else if (a is int aint)
                {
                    parts.Add((byte)'I');
                    AddAll(parts, BitConverter.GetBytes(aint));
                }
                else if (a is string astring)
                {
                    parts.Add((byte)'S');
                    parts.Add((byte)astring.Length);
                    AddAll(parts, Encoding.UTF8.GetBytes(astring));
                }
                else if (a is double adouble)
                {
                    parts.Add((byte)'D');
                    AddAll(parts, BitConverter.GetBytes(adouble));
                }
                else if (a is float afloat)
                {
                    parts.Add((byte)'F');
                    AddAll(parts, BitConverter.GetBytes(afloat));
                }
                else if (a is bool abool)
                {
                    parts.Add((byte)'B');
                    parts.Add((byte)(abool ? 1 : 0));
                }
                else if (a is byte v)
                {
                    parts.Add((byte)'b');
        ...
        private static int GetBytesNeeded(byte[] buffer, int bufferSize)
        {
        ...
        private static bool IsValidMessage(byte[] buffer, int bufferSize)
        {
